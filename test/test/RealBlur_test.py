...
parser.add_argument('--input_dir', type=str, default='./image_DBSGAN')
parser.add_argument('--gt_root', type=str, default='./dataset/RealBlur/RealBlur-R_BM3D_ECC_IMCORR_centroid_itensity_ref')
...
def evaluation_folder(args_list):
    ...
    for j, img_path in enumerate(imgList):
        ...
        crop = True
        if crop:
            blurred = blurred[0:736, 0:648, :]
            gt = gt[0:736, 0:648, :]

        crop_border = 0
        if crop_border != 0:
            blurred = blurred[crop_border:-crop_border, crop_border:-crop_border, ...]
            deblurred = deblurred[crop_border:-crop_border, crop_border:-crop_border, ...]
        ...
        deblur_ssim_map = calculate_ssim(aligned_xr1, aligned_deblurred, crop_border=0)
        ...
        # vis_img_out_name = 'vis_%s_blur_%s_PSNR_%5.5f_%5.5f_SSIM_%5.5f_%5.5f.jpg' % (
        # scene_name, img_name[:-4], blur_psnr, deblur_psnr, blur_ssim, deblur_ssim)
